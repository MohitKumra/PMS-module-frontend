...
const variantStyles: Record<Variant, string> = {
  primary:
    'text-text-onaccent shadow-md shadow-accent/10 hover:shadow-lg hover:shadow-accent/20 active:shadow-sm',
  secondary:
  ...
  ghost:
    'bg-transparent text-text-muted hover:bg-accent-subtle hover:text-accent',
  danger:
...
export function Button({
  variant = 'primary', size = 'md', loading = false, disabled, className = '',
  leftIcon, rightIcon, children, fullWidth = false, ...props
}: ButtonProps) {
  return (
    <button
      {...props}
